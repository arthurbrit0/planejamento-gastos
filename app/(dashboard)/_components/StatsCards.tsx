...
function StatsCards({from, to, configuracoesUser}: Props) {
  ...
  const saldo = entrada - saida;                                                            // calculamos o saldo do usuario, subtraindo o valor de saida do valor de entrada
  ...
  return (
    <div className="container relative flex w-full flex-wrap gap-2 md:flex-nowrap mx-auto">
      <SkeletonWrapper isLoading={querySaldo.isFetching}>
        <StatCard formatter={formatter} value={entrada} title={"Entradas"} icon={           // retornamos um card de estatísticas, passando o valor de entrada, o título e o ícone como props, alem do formatter da moeda
        <TrendingUp className="h-12 w-12 items-center rounded-lg p-2 text-emerald-700 bg-emerald-400 opacity-70" />
        } />
      </SkeletonWrapper>
      <SkeletonWrapper isLoading={querySaldo.isFetching}>
        <StatCard formatter={formatter} value={saida} title={"Saídas"} icon={               // retornamos um card de estatísticas, passando o valor de saida, o título e o ícone como props, alem do formatter da moeda
        <TrendingDown className="h-12 w-12 items-center rounded-lg p-2 text-rose-700 bg-rose-400 opacity-70" />
        } />
      </SkeletonWrapper>
      <SkeletonWrapper isLoading={querySaldo.isFetching}>
        <StatCard formatter={formatter} value={saldo} title={"Saldo"} icon={                // retornamos um card de estatísticas, passando o saldo, o título e o ícone como props, alem do formatter da moeda
